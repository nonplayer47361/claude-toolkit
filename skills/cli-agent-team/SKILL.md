...
**Step 1.5: 도구 스캔 — 사용 가능한 MCP · 스킬 인벤토리**

유형이 정해지면 즉시 아래 명령으로 환경을 파악한다:

```bash
claude mcp list 2>/dev/null          # 등록된 MCP 서버
ls ~/.claude/skills/ 2>/dev/null     # 설치된 스킬
ls mcp-servers/ 2>/dev/null          # 이 저장소 MCP 서버 (있는 경우)
```

결과를 프로젝트 유형과 연결해 활용 가능 도구와 외부 탐색이 필요한 도구로 분류한다:

| 필요 기능 | 먼저 확인 | 없으면 |
|----------|---------|------|
| 단계별 계획 추론 | `sequentialthinking` MCP | 없이 진행 |
| 세션 간 컨텍스트 보존 | `memory` MCP | BRIEF.md + PLAN.md로 대체 |
| 리서치 / 문서 수집 | browser / web search MCP | WebSearch 도구 직접 사용 |
| 코드 커밋 · PR | `git-helper` 스킬 | 수동 git 명령어 |
| 구현 후 리뷰 | `code-review-ko` 스킬 | Claude 직접 검토 |
| 외부 API 연동 | 해당 서비스 전용 MCP | 직접 API 호출 코드 |

사용자에게 한 줄 요약으로 제시한다:

```
🔍 도구 스캔:
  MCP: sequentialthinking ✅, memory ✅  (또는 ❌ 미설치)
  스킬: git-helper ✅, code-review-ko ✅
  이 프로젝트 권장: [sequentialthinking(계획), memory(세션 유지), git-helper(PR)]
  탐색 필요: [없음 / 예: Notion MCP — 문서 연동이 필요하면 검토]
```

- `sequentialthinking` MCP가 활성이면 Step 3 계획 구체화 시 활용한다 (복잡한 에픽 분해·의존성 분석).
- 탐색이 필요한 도구는 Step 3 통합 의존성 질문과 함께 다루고, 결론을 Step 5 BRIEF.md에 반영한다.
...
- **성공 정의가 없는가?** → "이것이 완성됐다고 느끼는 순간을 구체적으로 묘사해 주세요. 어떤 장면, 어떤 상태인가요?"
- **제약/금지 조건이 불명확한가?** → "하면 안 되거나 반드시 지켜야 할 것이 있나요? (특정 언어 고정, 서버리스 필수, 비용 상한 등)"
- **통합 의존성이 불명확한가?** → "외부 서비스, DB, 인증 중 이미 정해진 게 있나요?" (Step 1.5 스캔 결과와 연결해 제시)

**충분히 구체화된 기준** (아래 다섯 가지가 모두 답해지면 Step 4로):
1. 한 줄 정의를 쓸 수 있는가 (누가 / 무엇을 / 왜)
2. v1 범위가 명확한가 (넣을 것 vs 나중에 할 것)
3. 기술 스택이 결정됐는가
4. **성공 정의가 명확한가** — "완료"가 어떤 구체적인 상태인지 말할 수 있는가
5. **도구·통합 의존성이 확정됐는가** — Step 1.5 결과에서 쓸 도구, 필요한 외부 연동이 결정됐는가

"나중에 결정"도 결정으로 인정한다. 단 "언제/어떤 조건에서 결정할지"를 Step 5 BRIEF.md 열린
질문 항목에 반드시 기록해야 Step 4로 넘어간다.
...
- 한 줄 정의
- 기술 스택
- **성공 정의** (무엇이 달성되면 "완료"인가, 핵심 검증 방법)
- **도구 · 통합 의존성** (쓸 MCP/스킬, 외부 API/서비스 목록, 첫 버전 범위)
- 첫 번째 마일스톤 (MVP, 목표 기간 포함)
- 에픽 3~5개 + 권장 빌드 순서
- 열린 질문 (아직 결정 안 된 것 — 이 중 통합 의존성 미결 항목은 별도 표시)
...
**Step 1.5: 도구 스캔** (`claude mcp list`, `ls ~/.claude/skills/`)

0-A.1 Step 1.5와 동일하게 실행한다. 기존 프로젝트에 이미 MCP/스킬이 연동돼 있으면
"이미 사용 중" 표시, 이번 기능 추가로 새로 필요한 도구가 있으면 제안한다.
...
- 영향 받는 파일/모듈 범위는?
- 이 기능이 완성됐다는 것을 어떻게 확인할 수 있나? (성공 정의)
- 새로 필요한 외부 의존성이 생기는가? (Step 1.5 스캔 결과와 연결)
